...
if (isset($_GET['token'])) {
    $token = $_GET['token'];
    $query = "SELECT * FROM admin WHERE email_token = '$token' AND token_expiration > NOW()";
    ...
    if (mysqli_num_rows($result) > 0) {
        if (isset($_POST['updatePassword'])) {
            ...
            $updateQuery = "UPDATE admin SET password = '$hashedPassword', email_token = NULL, token_expiration = NULL WHERE email_token = '$token'";
            if (mysqli_query($db, $updateQuery)) {
                $message = "<div class='alert alert-success' role='alert'>Your password has been updated successfully. You will be redirected to the login page in 3 seconds.</div>";
            ...
            <div class="col-md-8 col-lg-6 col-xl-4 offset-xl-1">
                <form class="login-form" action="" method="POST">
                    ...
                    <div class="text-center text-lg-start mt-4 pt-2">
                        <button type="submit" name="updatePassword" class="btn btn-primary btn-lg" style="padding-left: 2.5rem; padding-right: 2.5rem;">Update Password</button>
                    </div>
                </form>
            </div>
        </div>
