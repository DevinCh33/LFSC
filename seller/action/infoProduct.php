...
// Extract quantity, weight, price and discount values from the arrays
$quantityValues = $formDataArray['quantity'];
...
if ($act == "add") {
    ...
    $sql = "INSERT INTO product (productCode, product_name, product_image, descr, categories_id, owner, product_date, lowStock, status)
            VALUES ('$productCode', '$productName','$urlImage', '$productDescription', '$productCategory', '$store', '$date', '$productStockAlert', '$productStatus')";
    ...
    if ($db->query($sql) === true) {
        ...
        foreach ($weightValues as $key => $weight) {
            ...
            $priceInsertSQL = "INSERT INTO tblprice (productID, proQuant, proWeight, proPrice, proDisc)
                                VALUES ('$lastProductID', '$quantity', '$weight', '$price', '$discount')";
...
elseif ($act == "edit") {
	...
	$updateSQL = "UPDATE product
					SET productCode = '$productCode', product_name = '$productName', descr = '$productDescription', categories_id = '$productCategory', status = '$productStatus'
	...
	foreach ($priceNo as $index => $price) {
		...
		if ($price == "true") {
			// New price
			$newPrice = "INSERT INTO tblprice (productID, proQuant, proWeight, 
			proPrice, proDisc) VALUES ('$productID', '$proQuant', 
			'$proWeight', '$proPrice', '$proDisc')";
			$db->query($newPrice);
		}

		else {
			// Use the retrieved values to update the database
			$updatePrice = "UPDATE tblprice SET proQuant = '$proQuant', proWeight = '$proWeight', proPrice = '$proPrice', proDisc = '$proDisc' WHERE priceNo = '$price'";
